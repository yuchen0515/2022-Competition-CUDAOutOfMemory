...
def random_replace_word(text, tokenizer, chinese_token_ids):
    try:
        ...
        replaced_text = tokenizer.convert_tokens_to_string(replaced_text).replace(' ', '')
...
def make_noises(text, tokenizer, chinese_token_ids):
    ...
    for _ in range(noises_count):
        ...
        while one_more:
            ...
            if noise_mode == 0:
                noise_text = random_replace_word(noise_text, tokenizer, chinese_token_ids)
...
for topic in corpus_topic:
    ...
    for idx, sentence in tqdm(enumerate(corpus), total=len(corpus)):
        ...
        noise_sentence_data['noise_sentence_list'] = make_noises(sentence, tokenizer, chinese_token_ids)
    ...
    for idx, noise_dict in tqdm(noise_data.items()):
        ...
        ground_truth_sentence_list.extend([noise_dict['ground_truth_sentence']] * noise_sentence_len)
